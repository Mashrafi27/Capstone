...
# def perform_nms(nms_predictions, scores, overlap, top_k):
#     """
#     Perform Non-Maximum Suppression (NMS) and return the indices of the kept boxes.

#     Args:
#         nms_predictions (Tensor): Bounding boxes of shape [N, 4] with format [x1, y1, x2, y2].
#         scores (Tensor): Scores for each bounding box of shape [N].
#         overlap (float): IoU threshold for suppression.
#         top_k (int): Maximum number of boxes to keep after NMS.

#     Returns:
#         Tensor: Indices of boxes to keep.
#         int: Number of boxes kept.
#         None: Placeholder for compatibility.
#     """
#     # Perform NMS
#     keep_indices = nms(nms_predictions, scores, overlap)

#     # Apply top_k filtering
#     if top_k is not None and top_k > 0:
#         keep_indices = keep_indices[:top_k]

#     # Number of boxes to keep
#     num_to_keep = len(keep_indices)

#     return keep_indices, num_to_keep, None
...
class CLRHead(nn.Module):
    ...
    def get_lanes(self, output, as_lanes=True):
        ...
        for predictions in output:
            ...
            # keep = keep.to("cuda:1")
            # num_to_keep = num_to_keep.to("cuda:1")
