...
class NeckWithConvs(torch.nn.Module):
    def __init__(self, neck):
        super(NeckWithConvs, self).__init__()
        self.neck = neck
...
class Runner(object):
    def __init__(self, cfg):
        ...
        # adding Darknet Backbone
        # depth = [1, 2, 2]
        # width = [3, 32, 64, 128, 256, 512]
        # backbone = DarkNet(width, depth)
        # backbone = backbone.cuda()
        # self.net.backbone = backbone 
        # end adding Darknet Backbone
        
        # adding pretrained Darknet Backbone
        ...
        darknet_backbone = darknet_backbone.cuda()
        self.net.backbone = darknet_backbone
        # end adding pretrained Darknet Backbone
        
        # adding pretrained DarkFPN neck
        darknet_neck = state_dict['model'].fpn
        darknet_neck = darknet_neck.float()
        darknet_neck = darknet_neck.cuda()
        
        self.net.neck = NeckWithConvs(darknet_neck)
        
        # end adding pretrained DarkFPN neck
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.cfg.epochs):
            ...
            # freezing neck parameters
            for param in self.net.module.neck.neck.parameters():
                param.requires_grad = False
