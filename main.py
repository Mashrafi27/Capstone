...
def main():
    ...
    best_obj = 0
    f_loss = open("all_loss_pcgrad_divergentneck_cross_stitch_scaled_slow_after_aggressive_v8n.txt", "a")
    f_scores = open("all_scores_pcgrad_divergentneck_cross_stitch_scaled_slow_after_aggressive_v8n.txt", "a")
...
#    'F1': 0.673}}}
    ...
    wandb.log({
        "metrics/precision":last_obj[0],
        "metrics/recall": last_obj[1],
        "metrics/mAP@50": last_obj[2],
        "metrics/mAP@50:95": last_obj[3]
    }, step=step)
    ...
    for epoch in range(0, args.epochs):
        ...
        # for i, data_obj in p_bar:
            ...
            # loss_lane = loss_lane * loss_scaler
...
# 
        ...
        if epoch <= 5:
            torch.save(net.state_dict(), f'./weights/pcgrad_divergentneck_cross_stitch_model_scaled_slow_after_aggressive_epoch{epoch}_v8n.pt')
        else:
            torch.save(net.state_dict(), './weights/pcgrad_divergentneck_cross_stitch_model_scaled_slow_after_aggressive_v8n_last.pt')
        if best_obj == last_obj[3]:
            torch.save(net.state_dict(), './weights/pcgrad_divergentneck_cross_stitch_model_scaled_slow_after_aggressive_v8n_best.pt')
