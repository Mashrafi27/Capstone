...
def main():
    ...
    state_dict = torch.load("/home/mmm9886/Desktop/Capstone_Implementation/YOLOv8-pt copy 3/weights/best.pt", map_location=torch.device(device))
    ...
    best_obj = 0
    f_loss = open("all_loss_vehicle_only_pcgrad3.txt", "a")
    f_scores = open("all_scores_vehicle_only_pcgrad3.txt", "a")
    ...
    for epoch in range(0, 15):
        ...
        torch.save(net.state_dict(), './weights/vehicle_pcgrad3_model_last.pt')
        if best_obj == last_obj[5]:
            torch.save(net.state_dict(), './weights/vehicle_pcgrad3_model_best.pt')
        
        # f_scores.write(f"tp: {last_obj[0]}, fp: {last_obj[1]}, precision: {last_obj[2]}, recall: {last_obj[3]}, mAP@50: {last_obj[4]}, mAP: {last_obj[5]}\n")
        f_scores.write(f"{last_obj[0]}, {last_obj[1]}, {last_obj[2]}, {last_obj[3]}, {last_obj[4]}, {last_obj[5]}\n")
...
def test_obj(args, params, device, model=None):
    ...
    for samples, targets, shapes in p_bar:
        ...
        outputs = util.non_max_suppression(outputs, 0.1, 0.65) # 0.001 originally
        # Metrics
