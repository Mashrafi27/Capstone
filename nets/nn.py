...
class DP(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1):
        ...
        # Ensure in_channels matches the input channels
...
class CSPPAN(torch.nn.Module):
    def __init__(self, width, depth):
        super().__init__()
        self.up = torch.nn.Upsample(scale_factor=2, mode='nearest')

        # Define the convolutions and CSP/DP blocks according to the CSP-PAN diagram
        self.conv_c5 = torch.nn.Conv2d(width[5], width[4], kernel_size=1)
        self.csp_c5 = CSP(width[4] + width[5], width[4], depth[0], False)
        
        self.conv_c4 = torch.nn.Conv2d(width[4], width[3], kernel_size=1)
        self.csp_c4 = CSP(width[3] + width[4], width[3], depth[0], False)

        self.conv_c3 = torch.nn.Conv2d(width[3], width[3], kernel_size=1)
        self.csp_c3 = CSP(width[3] + width[3], width[3], depth[0], False)
        
        self.dp1 = DP(width[4], width[5])
        self.dp2 = DP(width[4], width[5])

        # Downsampling layers for the P4/P5/P6 paths
        self.down_p4 = torch.nn.Conv2d(width[3], width[4], kernel_size=3, stride=2, padding=1)
        self.down_p5 = torch.nn.Conv2d(width[4], width[5], kernel_size=3, stride=2, padding=1)

    def forward(self, x):
        c3, c4, c5 = x  # Assume x is a tuple with feature maps (C3, C4, C5)

        # C5 -> P6
        p6 = self.dp1(c5)

        # C5 -> CSP -> P5
        h1 = self.csp_c5(torch.cat([self.up(p6), c5], 1))
        p5 = self.dp2(h1)

        # C4 -> CSP -> P4
        h2 = self.csp_c4(torch.cat([self.up(h1), c4], 1))
        
        # C3 -> CSP -> P3
        h3 = self.csp_c3(torch.cat([self.up(h2), c3], 1))

        # Downsample and apply CSP to form higher levels again
        h4 = self.csp_c4(torch.cat([self.down_p4(h3), h2], 1))
        h6 = self.csp_c5(torch.cat([self.down_p5(h4), p5], 1))
